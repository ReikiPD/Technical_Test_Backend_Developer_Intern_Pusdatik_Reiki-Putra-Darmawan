...
const createAttendance = async (req, res) => {
  try {
    ...
    // Jika statusnya selain Present (Sick, Leave, Absent), waktu check-in dan check-out TIDAK BOLEH diisi
    if (["Sick", "Leave", "Absent"].includes(status)) {
      if (check_in) {
        errors.check_in = [
          `Check-in time cannot be filled when status is ${status}.`,
        ];
      }
      if (check_out) {
        errors.check_out = [
          `Check-out time cannot be filled when status is ${status}.`,
        ];
      }
    }
    ...
    if (error.code === "23505") {
      return res.status(400).json({
        ...
        errors: {
          attendance_date: [
            "Attendance data for this employee on this date already exists.",
          ],
        },
      });
    }

    // Menangkap error Check Constraint Violation dari PostgreSQL (Kode 23514)
    if (error.code === "23514") {
      return res.status(400).json({
        success: false,
        ...
        errors: {
          database_rule: [
            "The submitted data violates database structural rules (e.g., invalid time for current status).",
          ],
        },
      });
    }
